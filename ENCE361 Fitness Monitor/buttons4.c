...
initButtons (void)
{
    ...
    //GPIOIntRegister(UP_BUT_PORT_BASE,UP_BUTPortEIntHandler);
    GPIOIntEnable(UP_BUT_PORT_BASE, UP_BUT_PIN);
    GPIOIntClear(UP_BUT_PORT_BASE, UP_BUT_INT_PIN);
    ...
    //GPIOIntRegister(DOWN_BUT_PORT_BASE,DOWN_BUTPortDIntHandler);
    GPIOIntEnable(DOWN_BUT_PORT_BASE, DOWN_BUT_PIN);
    GPIOIntClear(DOWN_BUT_PORT_BASE, DOWN_BUT_INT_PIN);
    ...
    GPIOIntTypeSet(LEFT_BUT_PORT_BASE, LEFT_BUT_PIN, GPIO_RISING_EDGE);
    but_normal[LEFT] = LEFT_BUT_NORMAL;
    //GPIOIntRegister(LEFT_BUT_PORT_BASE,L_BUTPortFIntHandler);
    GPIOIntEnable(LEFT_BUT_PORT_BASE, LEFT_BUT_PIN);
    GPIOIntClear(LEFT_BUT_PORT_BASE, LEFT_BUT_INT_PIN);
    ...
    GPIOIntTypeSet(RIGHT_BUT_PORT_BASE, RIGHT_BUT_PIN, GPIO_RISING_EDGE);
    //GPIOIntRegister(RIGHT_BUT_PORT_BASE,R_BUTPortFIntHandler);
    GPIOIntEnable(RIGHT_BUT_PORT_BASE, RIGHT_BUT_PIN);
    GPIOIntClear(RIGHT_BUT_PORT_BASE, RIGHT_BUT_INT_PIN);
...
void initSwitches() {
    SysCtlPeripheralEnable (SW_PERIPH); //enable GPIO port A which switches are connected

    GPIO_PORTF_LOCK_R = GPIO_LOCK_KEY;
    GPIO_PORTF_CR_R |= GPIO_PIN_7; //PF7 unlocked
    GPIO_PORTF_LOCK_R = GPIO_LOCK_M;

    GPIOPinTypeGPIOInput (SW_PORT_BASE, SW1_PIN); //set switch 1 as input
    GPIOPinTypeGPIOInput (SW_PORT_BASE, SW2_PIN); //set switch 2 as input

}

uint16_t checkSwitches(uint16_t Num_SW) {

    return (GPIOPinRead(SW_PORT_BASE, Num_SW));
}
